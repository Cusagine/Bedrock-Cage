...
data modify storage brc:map_info title set value "晚上"
# summary:地图摘要。会在载入地图时用作副标题。
data modify storage brc:map_info summary set value "非常烦人。"
# desc:地图描述。
data modify storage brc:map_info desc set value "some description"
# content:要加载的模板的命名空间ID。
data modify storage brc:map_info content set value "brc:night"
