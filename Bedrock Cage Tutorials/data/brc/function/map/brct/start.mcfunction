# id:当前地图的id。
data modify storage brc:map_info id set value "start"
# title:地图标题。会在载入地图时用作标题。
data modify storage brc:map_info title set value '{"translate":"map.brct.start.title","fallback":"The Start"}'
# summary:地图摘要。会在载入地图时用作副标题。
data modify storage brc:map_info summary set value '{"translate":"map.brct.start.summary","fallback":"It started."}'
# desc:地图描述。
data modify storage brc:map_info desc set value '{"translate":"map.brct.start.desc","fallback":""}'
# content:要加载的模板的命名空间ID。
data modify storage brc:map_info content set value "brct:start"
