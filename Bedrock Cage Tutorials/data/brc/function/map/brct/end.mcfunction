# id:当前地图的id。
data modify storage brc:map_info id set value "end"
# title:地图标题。会在载入地图时用作标题。
data modify storage brc:map_info title set value '{"translate":"map.brct.end.title","fallback":"The End"}'
# summary:地图摘要。会在载入地图时用作副标题。
data modify storage brc:map_info summary set value '{"translate":"map.brct.end.summary","fallback":"Maybe the End?"}'
# desc:地图描述。
data modify storage brc:map_info desc set value '{"translate":"map.brct.end.desc","fallback":""}'
# content:要加载的模板的命名空间ID。
data modify storage brc:map_info content set value "brct:end"
# large:地图是否占4个区块。 （true——4个 false——1个）
data modify storage brc:map_info large set value true
# next_pool:地图使用的下一个抽取池。
data modify storage brc:map_info next_pool set value "to_menu"
