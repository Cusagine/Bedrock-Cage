...
data modify storage brc:map_info id set value "gulinhar_future"
# title:地图标题。会在载入地图时用作标题。
data modify storage brc:map_info title set value "谷林哈尔的未来"
# summary:地图摘要。会在载入地图时用作副标题。
data modify storage brc:map_info summary set value "什么都看不见。你在哪？"
# desc:地图描述。
data modify storage brc:map_info desc set value "月亮不会凭空消失，所有人都这么认为。然后，月亮消失了，它的存在也变成了传说。"
# content:要加载的模板的命名空间ID。
data modify storage brc:map_info content set value "brc:gulinhar_future"
