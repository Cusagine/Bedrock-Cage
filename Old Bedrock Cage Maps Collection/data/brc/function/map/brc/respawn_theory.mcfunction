...
data modify storage brc:map_info id set value "respawn_theory"
# title:地图标题。会在载入地图时用作标题。
data modify storage brc:map_info title set value "重生理论"
# summary:地图摘要。会在载入地图时用作副标题。
data modify storage brc:map_info summary set value "生命是运动的存在。"
# desc:地图描述。
data modify storage brc:map_info desc set value "早已死去的生灵仍在依靠程序生产运动，使它不会死亡。"
# content:要加载的模板的命名空间ID。
data modify storage brc:map_info content set value "brc:respawn_theory"
