...
data modify storage brc:map_info id set value "swellhead_abyss"
# title:地图标题。会在载入地图时用作标题。
data modify storage brc:map_info title set value "自负者深渊"
# summary:地图摘要。会在载入地图时用作副标题。
data modify storage brc:map_info summary set value "跨过这个峡谷并不困难。"
# desc:地图描述。
data modify storage brc:map_info desc set value "偏执使事物困难。这不荒谬吗？这不可惜吗？"
# content:要加载的模板的命名空间ID。
data modify storage brc:map_info content set value "brc:swellhead_abyss"
