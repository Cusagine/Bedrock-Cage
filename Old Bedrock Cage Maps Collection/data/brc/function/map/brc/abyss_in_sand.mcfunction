...
data modify storage brc:map_info id set value "abyss_in_sand"
# title:地图标题。会在载入地图时用作标题。
data modify storage brc:map_info title set value "沙中深渊"
# summary:地图摘要。会在载入地图时用作副标题。
data modify storage brc:map_info summary set value "请您不要紧张。"
# desc:地图描述。
data modify storage brc:map_info desc set value "人生需要小心，但我们并没有用来小心的时间。"
# content:要加载的模板的命名空间ID。
data modify storage brc:map_info content set value "brc:abyss_in_sand"
