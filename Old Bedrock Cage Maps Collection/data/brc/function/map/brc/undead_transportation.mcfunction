...
data modify storage brc:map_info id set value "undead_transportation"
# title:地图标题。会在载入地图时用作标题。
data modify storage brc:map_info title set value "亡灵交通"
# summary:地图摘要。会在载入地图时用作副标题。
data modify storage brc:map_info summary set value "总是像这样匆忙。"
# desc:地图描述。
data modify storage brc:map_info desc set value "在以前，地下矿井非常忙碌；而现在，地下矿井和以前一样忙碌。"
# content:要加载的模板的命名空间ID。
data modify storage brc:map_info content set value "brc:undead_transportation"
