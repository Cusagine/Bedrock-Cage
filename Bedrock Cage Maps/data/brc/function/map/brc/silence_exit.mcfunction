...
data modify storage brc:map_info id set value "silence_exit"
# title:地图标题。会在载入地图时用作标题。
data modify storage brc:map_info title set value "无声出口"
# summary:地图摘要。会在载入地图时用作副标题。
data modify storage brc:map_info summary set value "请安静离开。"
# desc:地图描述。
data modify storage brc:map_info desc set value "演员已经离场，请安静离开。如果您做不到，请不要离开，并等待保安进场。"
# content:要加载的模板的命名空间ID。
data modify storage brc:map_info content set value "brc:silence_exit"
