...
data modify storage brc:map_info id set value "2nd_survive_rule"
# title:地图标题。会在载入地图时用作标题。
data modify storage brc:map_info title set value "第二生存法则"
# summary:地图摘要。会在载入地图时用作副标题。
data modify storage brc:map_info summary set value "你可以做任何有益于生存的事。"
# desc:地图描述。
data modify storage brc:map_info desc set value "第一生存法则：活下去。第二生存法则：在此基础上，你可以做任何事情。"
# content:要加载的模板的命名空间ID。
data modify storage brc:map_info content set value "brc:2nd_survive_rule"
