...
data modify storage brc:map_info id set value "withering_nature"
# title:地图标题。会在载入地图时用作标题。
data modify storage brc:map_info title set value "凋亡的自然"
# summary:地图摘要。会在载入地图时用作副标题。
data modify storage brc:map_info summary set value "一切都会成为过去。"
# desc:地图描述。
data modify storage brc:map_info desc set value "一切都会成为过去，只有阳光依旧。"
# content:要加载的模板的命名空间ID。
data modify storage brc:map_info content set value "brc:withering_nature"
