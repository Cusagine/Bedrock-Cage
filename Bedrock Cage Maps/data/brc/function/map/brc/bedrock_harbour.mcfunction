...
data modify storage brc:map_info id set value "bedrock_harbour"
# title:地图标题。会在载入地图时用作标题。
data modify storage brc:map_info title set value "基岩港湾"
# summary:地图摘要。会在载入地图时用作副标题。
data modify storage brc:map_info summary set value "有的人希望为冒险家提供一个温暖的港湾。"
# desc:地图描述。
data modify storage brc:map_info desc set value "港湾为迷失之人提供慰藉。（你可以在这里休整。但请不要做什么过分的事情……）"
