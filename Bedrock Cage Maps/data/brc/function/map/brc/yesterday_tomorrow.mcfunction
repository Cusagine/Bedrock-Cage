...
data modify storage brc:map_info id set value "yesterday_tomorrow"
# title:地图标题。会在载入地图时用作标题。
data modify storage brc:map_info title set value "昨天的明天"
# summary:地图摘要。会在载入地图时用作副标题。
data modify storage brc:map_info summary set value "不是今天。"
# desc:地图描述。
data modify storage brc:map_info desc set value "希望就在明天，但现在并不是明天。"
# content:要加载的模板的命名空间ID。
data modify storage brc:map_info content set value "brc:yesterday_tomorrow"
