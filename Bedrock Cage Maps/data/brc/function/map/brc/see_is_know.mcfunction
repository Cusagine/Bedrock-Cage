...
data modify storage brc:map_info id set value "see_is_know"
# title:地图标题。会在载入地图时用作标题。
data modify storage brc:map_info title set value "所见即所知"
# summary:地图摘要。会在载入地图时用作副标题。
data modify storage brc:map_info summary set value "看不见即为不存在。"
# desc:地图描述。
data modify storage brc:map_info desc set value "看不见的东西不会被意识到，自然也不会被进一步了解——但是它存在。"
# content:要加载的模板的命名空间ID。
data modify storage brc:map_info content set value "brc:see_is_know"
